...
The main purpose of the `wsgw` application is to make possible the passing messages among websocket and NATS clients.

Depending on the working mode, that is controlled by tha starting command, this application can act in the following roles:
...
The figure below demonstrates the detailed architecture of a relatively complex use-case,
...
```bash
    ...
    Options:
          --version   Show version number                                  [boolean]
          --help      Show help                                            [boolean]
      -c, --config    The name of the configuration file     [default: "config.yml"]
      -p, --port      The webSocket server port             [number] [default: 8001]
      -i, --inbound   Comma separated list of inbound NATS topics to forward through
...
```bash
    ...
    Options:
      ...
      -s, --source        The name of the YAML or JSON format source file that holds
                          the messages to send                       [default: null]
